...
```{r}
data %>% summarise(max = max(average_steps))

max_interval<-data[which.max(data$average_steps),1]
...
ggplot(data, aes(x=interval, y=average_steps2)) + geom_line(aes(color = dayoweek, linetype = dayoweek))
```
