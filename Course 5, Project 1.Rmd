...
```{r}
imput_function <- function(x, data) {
  for (i in x) {
if(is.na(data[i,1])) {
data[i,1]<-average_steps$average_steps[average_steps$interval==data[i,3]]
  } else {
  data[i,1]<-data[i,1]
  }
    return(data)
...
data[7,]
if(is.na(data[7,1])) {
  data[7,1]<-average_steps$average_steps[average_steps$interval==data[7,3]]
} else {
  data[7,1] <- data[7,1]
}
data[7,]
